...
describe Runner do
  ...
  it 'prints usage info to stderr for no arguments' do
    error = StringIO.new
    Runner.run( [], out: StringIO.new, err: error )
    error.string.should include 'hostspec'
  end

  it 'prints usage info to stderr for help (-h/--help) switch' do
  ...
  it 'accepts host specs for arguments before command (-c/--cmd) switch'

  it 'executes the command (-c/--cmd) switch argument as a shell command'
